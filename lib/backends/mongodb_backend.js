...
function MongoDBConnector() {
  ...
  global.storraConfig.mergeDefaultsIntoCurrentConfiguration({
    mongodb: {
  ...
  log.debug('MongoDB configuration, merged with defaults:\n' +
      JSON.stringify(global.storraConfig.mongodb))

  var self = this
  this.maxRetries = global.storraConfig.mongodb.connectionMaxRetries
  this.timeBetweenRetries =
      global.storraConfig.mongodb.connectionTimeBetweenRetries
  this.database = global.storraConfig.mongodb.database
  ...
  // Also cache collection objects? But maybe it's cheap to access them every
  // time without using a cache (and probably safer)?
  ...
  this.remove = function(collectionName, key, writeResponse) {
    ...
    openConnection(function(err, mongoClient) {
      withCollectionDo(mongoClient, collectionName, function(collection) {
        collection.remove({_id: new ObjectID(key)},
