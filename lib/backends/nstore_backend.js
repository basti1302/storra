...
function NStoreConnector() {
  ...
  this.removeCollection = function removeCollection(collectionName,
    ...
    fs.exists(file, function (exists) {
      if (exists) {
        ...
        log.debug('%s does not exist, doing nothing.', file)
        writeResponse(null)
      }
    })
  }
  ...
  function withCollectionDo(collectionName, callback) {
    ...
    if (collection) {
      log.debug('accessing collection %s via cached collection object.', name)
      callback(collection)
    } else {
      collection = nStore.new(name, function (err) {
        log.debug('collection %s was not in cache.', name)
        if (err) { throw err }
