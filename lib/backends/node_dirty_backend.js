...
function NodeDirtyConnector() {
    ...
    withCollectionDo(collectionName, function(collection) {
      collection.forEach(function(key, doc) {
        if (doc) {
          ...
          log.warn('node-dirty backend handed empty doc to callback, key: ' +
              key)
        }
      })
      writeEnd(null)
  ...
  this.update = function(collectionName, key, doc, writeResponse) {
    ...
    withCollectionDo(collectionName, function(collection) {
      ...
      if (existingDoc) {
        log.debug('now really updating item ' + collectionName + '/' + key +
            ': ' + JSON.stringify(doc))
        collection.set(key, doc)
  ...
  function accessCollection(collectionName, callback) {
    ...
    if (collection) {
      log.debug('accessing collection ' + databaseFilename +
          ' via cached collection object.')
      if (callback) {
        callback(collection)
      }
      return collection
