...
function RequestHandler() {
  ...
  var BackendConnector = require(global.storraConfig.core.backend)
  ...
  function writeContentHeader(response, status, contentType,
      additionalHeaders) {
    writeHeader(response, status, merge({ 'Content-Type': contentType },
        additionalHeaders))
  }
  ...
  function writeJsonHeader(response, status, additionalHeaders) {
    writeContentHeader(response, status, 'application/json;charset=UTF-8',
        additionalHeaders)
  }
    ...
    createOrUpdate(request, response, function(bodyObject) {
      backend.create(collection, bodyObject, function(err, key) {
        if (err) {
          ...
          response.end()
          log.debug('successfully inserted entry into ' + collection +
              ' -> ' + key)
        }
      })
    })
